...
if ($fund_id > 0) {
    ...
    if ($fund) {
        ...
        $stmtItems->execute([$fund_id]);
        $fund_items = $stmtItems->fetchAll(PDO::FETCH_ASSOC);
    } else {
        $error_msg = "Fund not found.";
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $fund) {
    ...
    if (empty($title) || $totalAmount <= 0 || empty($date_released) || empty($received_by)) {
        ...
        try {
            ...
            // 1. Update Fund Record
            $stmt = $pdo->prepare("UPDATE funds SET title = ?, amount = ?, date_released = ?, received_by = ?, description = ? WHERE id = ?");
            $stmt->execute([$title, $totalAmount, $date_released, $received_by, $description, $fund_id]);

            // 2. Refresh Line Items (Delete and re-insert for simplicity)
            $stmtDel = $pdo->prepare("DELETE FROM fund_items WHERE fund_id = ?");
            $stmtDel->execute([$fund_id]);
            ...
            $pdo->commit();
            $success_msg = 'Fund release updated successfully!';
...
<div class="max-w-4xl mx-auto">
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-2xl font-bold text-white">Edit Fund Release</h2>
        <a href="report.php?fund_id=<?php echo $fund_id; ?>" class="text-sm text-gray-400 hover:text-white transition-colors">&larr; Back to Report</a>
    </div>
    ...
    <form method="POST" action="" class="space-y-6">
        ...
        <div class="bg-gray-800 rounded-xl shadow-lg border border-gray-700 p-6 space-y-6">
            ...
            <div>
                ...
                <input type="text" name="title" required value="<?php echo htmlspecialchars($fund['title']); ?>"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="date" name="date_released" required value="<?php echo $fund['date_released']; ?>"
                ...
                <div>
                    ...
                    <input type="text" name="received_by" required value="<?php echo htmlspecialchars($fund['received_by']); ?>"
                        class="w-full bg-gray-900 border border-gray-700 rounded-lg px-4 py-2.5 text-white focus:ring-2 focus:ring-accent-500 focus:border-transparent outline-none transition-all placeholder-gray-600">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="description" rows="2" placeholder="Additional details..."
                    class="w-full bg-gray-900 border border-gray-700 rounded-lg px-4 py-2.5 text-white focus:ring-2 focus:ring-accent-500 focus:border-transparent outline-none transition-all placeholder-gray-600"><?php echo htmlspecialchars($fund['description']); ?></textarea>
            </div>
        </div>
        ...
        <div class="bg-gray-800 rounded-xl shadow-lg border border-gray-700 p-6">
            <div class="flex justify-between items-center border-b border-gray-700 pb-2 mb-4">
                ...
                <div class="text-right">
                    ...
                    <span id="displayTotal" class="text-xl font-bold text-emerald-400">₦<?php echo number_format($fund['amount'], 2); ?></span>
                </div>
            </div>
            ...
            <div id="lineItemsContainer" class="space-y-3">
                ...
                <div class="line-item-row flex items-center gap-4">
                    <div class="flex-grow">
                        <input type="text" name="item_desc[]" required value="<?php echo htmlspecialchars($item['description']); ?>"
                    ...
                    <div class="w-40">
                        <input type="number" step="0.01" name="item_amount[]" required value="<?php echo $item['amount']; ?>" oninput="calculateTotal()"
