...
<div class="max-w-4xl mx-auto mb-6 no-print">
    <div class="flex flex-col md:flex-row items-center justify-between gap-4 bg-gray-800 p-4 rounded-xl border border-gray-700 shadow-lg">
        <div>
            <h2 class="text-xl font-bold text-white">Outstanding Tasks Report</h2>
            <a href="completed_task_report.php" class="text-xs text-accent-400 hover:underline">Switch to Completed Tasks &rarr;</a>
        </div>
        <div class="flex gap-2">
            <button onclick="exportToJPG()" class="bg-gray-700 hover:bg-gray-600 text-white px-4 py-2 rounded-lg flex items-center transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 mr-2">
                  <path stroke-linecap="round" stroke-linejoin="round" d="m2.25 15.75 5.159-5.159a2.25 2.25 0 0 1 3.182 0l5.159 5.159m-1.5-1.5 1.409-1.409a2.25 2.25 0 0 1 3.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 0 0 1.5-1.5V6a1.5 1.5 0 0 0-1.5-1.5H3.75A1.5 1.5 0 0 0 2.25 6v12a1.5 1.5 0 0 0 1.5 1.5Zm10.5-11.25h.008v.008h-.008V8.25Zm.375 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Z" />
                </svg>
                Export JPG
            </button>
            <button onclick="exportToPDF()" class="bg-gray-700 hover:bg-gray-600 text-white px-4 py-2 rounded-lg flex items-center transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 mr-2">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m.75 12 3 3m0 0 3-3m-3 3v-6m-1.5-9H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
                </svg>
                Export PDF
            </button>
            <button onclick="exportToWhatsApp()" class="bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-2 rounded-lg flex items-center transition-colors shadow-lg">
...
<div id="report-content" class="max-w-4xl mx-auto bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 p-8 shadow-xl rounded-xl border border-gray-700 print:shadow-none print:border-0 print:p-0">
    ...
    <div class="border-b border-gray-200 dark:border-gray-700 pb-6 mb-6">
        <div class="flex justify-between items-start">
            <div>
                <h1 class="text-2xl font-bold uppercase tracking-wide">Outstanding Tasks Report</h1>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 print:grid-cols-3">
        <div class="p-4 rounded-lg bg-gray-50 dark:bg-gray-700/30 border border-gray-100 dark:border-gray-600">
            <span class="block text-xs font-bold text-gray-500 uppercase">Total Outstanding</span>
            <span class="text-2xl font-bold"><?php echo count($tasks); ?></span>
        </div>
        <div class="p-4 rounded-lg bg-rose-50 dark:bg-rose-900/10 border border-rose-100 dark:border-rose-500/20">
            <span class="block text-xs font-bold text-rose-600 dark:text-rose-400 uppercase">High Priority</span>
            <span class="text-2xl font-bold text-rose-700 dark:text-rose-300">
                <?php echo count(array_filter($tasks, fn($t) => $t['priority'] === 'High')); ?>
            </span>
        </div>
        <div class="p-4 rounded-lg bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-500/20">
            <span class="block text-xs font-bold text-blue-600 dark:text-blue-400 uppercase">In Progress</span>
            <span class="text-2xl font-bold text-blue-700 dark:text-blue-300">
                <?php echo count(array_filter($tasks, fn($t) => $t['status'] === 'In Progress')); ?>
            </span>
        </div>
    </div>
    ...
    <?php if (count($tasks) > 0): ?>
        <?php foreach ($groupedTasks as $location => $locTasks): ?>
            <div class="mt-8 mb-4">
                <h3 class="text-sm font-black uppercase tracking-widest text-accent-600 bg-accent-500/5 px-3 py-1 rounded inline-block border border-accent-500/20 mb-4"><?php echo htmlspecialchars($location); ?></h3>
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="border-b border-gray-100 dark:border-gray-700">
                            <th class="py-2 text-[10px] font-bold text-gray-400 uppercase tracking-tighter w-1/2">Task Item</th>
                            <th class="py-2 text-[10px] font-bold text-gray-400 uppercase tracking-tighter">Status Details</th>
                        ...
                        <?php foreach ($locTasks as $task): ?>
                            <tr>
                                ...
                                <td class="py-3 text-xs text-gray-600 dark:text-gray-400">
                                    <?php echo htmlspecialchars($task['status_details'] ?? 'No details provided.'); ?>
                                </td>
                                <td class="py-3 text-center">
                                    <span class="status-badge text-[10px] px-1.5 py-0.5 rounded font-black uppercase transition-colors <?php 
                                        if ($task['status'] == 'Pending') echo 'text-blue-500';
                                        if ($task['status'] == 'In Progress') echo 'text-accent-500';
                                        if ($task['status'] == 'Completed') echo 'text-emerald-500';
                                    ?>">
        ...
        <?php endforeach; ?>
    <?php else: ?>
        <div class="py-12 text-center text-gray-500 italic">No outstanding tasks found.</div>
    <?php endif; ?>
...
async function exportReport(format) {
    ...
    darkElements.forEach(el => {
        ...
        // Remove dark classes and add light equivalents for capture
        el.classList.remove('dark:bg-gray-800', 'dark:bg-gray-700/30', 'dark:bg-rose-900/10', 'dark:bg-blue-900/10', 'dark:text-gray-300', 'dark:text-gray-400', 'dark:text-white', 'dark:border-gray-600', 'dark:divide-gray-700', 'bg-gray-100', 'dark:bg-gray-700');
        ...
        // Specific fix for status badges: ensure no background and proper text color
    ...
    try {
        ...
        if (format === 'jpg') {
            ...
            link.download = `Outstanding_Tasks_Report_${new Date().toISOString().split('T')[0]}.jpg`;
            ...
            pdf.save(`Outstanding_Tasks_Report_${new Date().toISOString().split('T')[0]}.pdf`);
        }
    } catch (error) {
...
function exportToWhatsApp() {
    let text = "*OUTSTANDING TASKS REPORT*\n";
    ...
    for (const [location, locTasks] of Object.entries(grouped)) {
        locTasks.forEach((task) => {
            ...
            // Format: Owner/Location - [Task number][Task Location/Dept] Bold
            text += `*${taskIndex}. ${location}*\n`;
            
            // Format: Title - [Task Title] [normal]
            text += `${task.task_name}\n`;
            
            // Format: Status - [Status (completed,etc][with icon][normal]
            text += `${task.status} ${statusEmoji}\n`;
            
            // Format: Status Details [Status Details][italics]
    ...
    text += "--------------------------------\n";
    text += "🔗 *Full Details:* https://wcs.afmweca.com/tasks/task_report.php";
...
function copyWhatsAppText() {
    ...
    alert('Report text copied! You can now paste it into WhatsApp.');
}
</script>
...
<div id="whatsapp-modal" class="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4 hidden">
    <div class="bg-gray-800 border border-gray-700 rounded-2xl w-full max-w-lg shadow-2xl">
        ...
        <div class="p-6">
            <p class="text-gray-400 text-sm mb-4">The text below is formatted for WhatsApp. Copy and paste it into your chat.</p>
            ...
            <div class="mt-6 flex gap-3">
                ...
                <button onclick="copyWhatsAppText()" class="flex-[2] bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3 rounded-xl shadow-lg transition-all transform active:scale-95 flex items-center justify-center">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 mr-2">
